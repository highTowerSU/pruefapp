...
$checks = [
    ...
    [str_contains($maintenance, 'archiveExactImportDuplicates') && str_contains($maintenance, 'historisch_nicht_eindeutig') && str_contains($schema, 'duplicate_of_inspection_id') && str_contains($worker, "'inspection_duplicate_archive'") && str_contains($cron, 'inspection-duplicate-archive:v3'), 'Eindeutige Re-Importdubletten werden nicht revisionssicher archiviert und aus aktiven Rechnungszuordnungen freigegeben.'],
