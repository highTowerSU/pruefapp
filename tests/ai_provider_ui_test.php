...
foreach ([
    ...
    [str_contains($template, 'hx-post=') && str_contains($template, 'OAuth 2.0 statt API-Token verwenden') && str_contains($template, 'data-search-select') && str_contains($template, 'OVHcloud AI Endpoints') && str_contains($template, 'IONOS AI Model Hub'), 'Die KI-Providerkarte ist nicht kompakt per HTMX, Modell-Auswahl und Provider-Voreinstellungen umgesetzt.'],
