...
foreach ([
    ...
    [str_contains($template, 'hx-post=') && str_contains($template, 'OAuth 2.0 statt API-Token verwenden') && str_contains($template, 'ai-provider-model-options') && str_contains($template, 'KI-Testanfrage') && str_contains($service, 'function diagnose') && str_contains($template, "vocabulary_ai_model: 'gpt-5-mini'") && str_contains($template, "vocabulary_ai_model: 'mistral@latest'") && str_contains($template, 'data-ai-recommended-model') && str_contains($template, 'data-no-search') && str_contains($template, 'OVHcloud AI Endpoints') && str_contains($template, 'IONOS AI Model Hub'), 'Die KI-Providerkarte ist nicht kompakt per HTMX, editierbarer Modell-Auswahl, Diagnose und Provider-Voreinstellungen umgesetzt.'],
