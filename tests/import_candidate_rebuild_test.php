...
try {
    ...
    if (($result['automatic'] ?? 0) !== 1 || ($result['manual_kept'] ?? 0) !== 4 || ($result['number_missing'] ?? 0) !== 1) throw new RuntimeException('Gesammelte Kandidaten wurden nicht erst nach der quellenübergreifenden Gruppierung bewertet.');
    if ((int) R::getCell("SELECT COUNT(*) FROM inspection WHERE source_type='manual'") !== 4 || (int) R::getCell("SELECT COUNT(*) FROM inspection WHERE source_type='reconciled'") !== 1 || (int) R::getCell("SELECT COUNT(*) FROM inspection WHERE external_number='001-23'") !== 0) throw new RuntimeException('Der Neuaufbau bewahrt echte Prüfweb-Prüfungen nicht oder entfernt Altimporte ohne Gerät nicht.');
