...
try {
    ...
    if ((int) R::getCell("SELECT COUNT(*) FROM inspection WHERE source_type='manual'") !== 2 || (int) R::getCell("SELECT COUNT(*) FROM inspection WHERE source_type='json'") !== 0 || (int) R::getCell("SELECT COUNT(*) FROM inspection WHERE source_type='reconciled'") !== 1) throw new RuntimeException('Der Neuaufbau bewahrt manuelle Prüfungen nicht oder entfernt alte Kandidatenimporte nicht.');
