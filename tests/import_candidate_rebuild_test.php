...
try {
    ...
    $failedDevice = R::dispense('device'); $failedDevice->room_id = 0; $failedDevice->name = 'Prüfweb-Gerät Sichtprüfung'; $failedDevice->external_number = '1000003'; $failedDevice->warming_device = 0; $failedDeviceId = (int) R::store($failedDevice);
    $failedManual = R::dispense('inspection'); $failedManual->device_id = $failedDeviceId; $failedManual->dedupe_key = 'manual-failed'; $failedManual->public_id = 'prf-manual-failed'; $failedManual->source_type = 'manual'; $failedManual->external_number = 'WEB-3'; $failedManual->test_date = '2026-08-12'; $failedManual->inspection_type = 'SK1'; $failedManual->storage_slot = '004'; $failedManual->result_status = 'failed'; R::store($failedManual);
    $slotDevice = R::dispense('device'); $slotDevice->room_id = 0; $slotDevice->name = 'Prüfweb-Speicherplatz'; $slotDevice->external_number = '1000004'; $slotDevice->warming_device = 0; $slotDeviceId = (int) R::store($slotDevice);
    $slotManual = R::dispense('inspection'); $slotManual->device_id = $slotDeviceId; $slotManual->dedupe_key = 'manual-slot'; $slotManual->public_id = 'prf-manual-slot'; $slotManual->source_type = 'manual'; $slotManual->external_number = 'WEB-4'; $slotManual->test_date = '2026-08-13'; $slotManual->inspection_type = 'SK1'; $slotManual->storage_slot = '045'; $slotManual->result_status = 'failed'; R::store($slotManual);
    ...
    if ($paddedSlotGroupedSources !== 2 || (string) R::getCell('SELECT result_status FROM inspection WHERE id=?', [(int) $slotManual->id]) !== 'failed') throw new RuntimeException('Speicherplätze mit führenden Nullen müssen eine eindeutige manuelle Prüfung sicher ergänzen, ohne deren Sichtprüfungsfehler zu überschreiben.');
