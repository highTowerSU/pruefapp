...
$checks = [
    ...
    [str_contains($searchSelect, "querySelectorAll('select:not([data-no-search])')") && str_contains($searchSelect, "plugins: ['dropdown_input']") && str_contains($searchSelect, 'if (select.tomselect)') && str_contains($searchSelect, 'externalLabel') && substr_count($template, 'plugins: [\'dropdown_input\']') >= 1 && !str_contains($template, 'createOnBlur: true') && str_contains($template, 'initializeDeviceVocabulary'), 'Geräte-Stammdaten verwenden nicht die einheitliche Auswahl ohne Anlage beim Verlassen.'],
    ...
    [str_contains($template, 'stammdaten-aus-letzter-pruefung') && str_contains($deviceController, 'copyLatestInspectionData') && str_contains($deviceController, 'geraet_stammdaten_aus_pruefung_uebernommen'), 'Die Übernahme von Geräte-Stammdaten aus der letzten Prüfung fehlt.'],
