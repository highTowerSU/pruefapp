...
$checks = [
    ...
    [str_contains($template, "[['manufacturer', 'Hersteller'], ['device_model', 'Typ / Modell'], ['name', 'Gerätebezeichnung']]") && str_contains($template, "refresh('manufacturer'") && str_contains($template, "refresh('device_model'") && str_contains($template, "selects.manufacturer?.on('change'") && str_contains($template, "selects.device_model?.on('change'"), 'Hersteller und Modell werden nicht als abhängige TomSelect-Auswahl initialisiert.'],
    ...
    [str_contains($template, 'data-action-nav="Geräteaktionen"') && str_contains($template, 'id="device-actions-panel"') && str_contains($template, 'class="card mb-3"><summary class="card-header fw-semibold">Auswahl &amp; Massenaktionen') && str_contains($template, 'data-action-nav="Neues Gerät"') && str_contains($template, 'data-action-nav="Neue Prüfung"'), 'Die Geräteaktionen oder Schnellzugriffe sind nicht korrekt markiert oder eingeklappt.'],
