...
$checks = [
    ...
    [str_contains($renderer, "\$hxSelect = ''") && str_contains($template, "{target:'#billing-content', swap:'outerHTML'"), 'HTMX responses must swap their single root without selecting a second fragment.'],
    [str_contains($deviceController, "if (\$isHx) return [200") && str_contains($renderer, "'#device-list-panel'") && str_contains($renderer, 'change from:select delay:120ms') && str_contains($renderer, 'Filter wird angewendet'), 'Device HTMX partial rendering or immediate filter feedback is missing.'],
