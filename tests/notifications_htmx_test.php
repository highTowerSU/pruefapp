...
$checks = [
    ...
    [str_contains($controller, 'notificationDropdownFragment') && str_contains($controller, 'downloads-notifications') && str_contains($controller, "'oob' => true"), 'Gelesen-Aktionen aktualisieren den Glockenzähler nicht mit.'],
    ...
    [str_contains($dropdown, 'hx-trigger="shown.bs.dropdown from:#notificationsDropdown"') && str_contains($dropdown, 'hx-post='), 'Das Benachrichtigungsmenü lädt oder aktualisiert nicht per HTMX.'],
    [str_contains($list, 'hx-target="#downloads-notifications"') && str_contains($list, 'hx-swap="outerHTML"') && str_contains($dropdown, 'hx-swap-oob="outerHTML"'), 'Die Download-Übersicht aktualisiert Benachrichtigungen nicht ohne Seitenreload.'],
];
