...
<?php $cronTodoCount = count($cronPendingJobs) + ((int) ($cronMissingReports ?? 0) > 0 ? 1 : 0) + (!empty($cronPdfMigrationPending) ? 1 : 0) + (!empty($cronPhoenixRestorePending) ? 1 : 0); ?><?php if ($cronTodoCount > 0): ?><div class="card border-info mb-3"><div class="card-header py-2 d-flex justify-content-between align-items-center"><strong><i class="fa-solid fa-list-check me-1" aria-hidden="true"></i>Ausstehende Hintergrundaufgaben</strong><span class="badge text-bg-info"><?= $cronTodoCount ?></span></div><div class="table-responsive"><table class="table table-sm mb-0 align-middle"><thead><tr><th>Aufgabe</th><th>Status</th><th>Fortschritt</th><th>Gerät</th><th>Meldung</th></tr></thead><tbody><?php if (!empty($cronPhoenixRestorePending)): ?><tr><td>Original-PDFs wiederherstellen</td><td><span class="badge text-bg-warning text-dark">Wartet</span></td><td><?= (int) ($cronPhoenixRestoreRemaining ?? 0) ?> offen</td><td>—</td><td>Die ursprünglichen Phoenix-Prüfberichte werden beim nächsten Hintergrundlauf zurückkopiert.</td></tr><?php endif; ?><?php if (!empty($cronPdfMigrationPending)): ?><tr><td>PDF-Aufbereitung</td><td><span class="badge text-bg-warning text-dark">Wartet</span></td><td><?= (int) ($cronPdfMigrationRemaining ?? 0) ?> offen</td><td>—</td><td>Bestehende Prüfberichte werden beim nächsten Hintergrundlauf mit dem aktuellen Layout neu erzeugt.</td></tr><?php endif; ?><?php if ((int) ($cronMissingReports ?? 0) > 0): ?><tr><td>Fehlende Prüfberichte</td><td><span class="badge text-bg-warning text-dark">Wartet</span></td><td>—</td><td>—</td><td><?= (int) $cronMissingReports ?> abgeschlossene Prüfungen haben noch keinen Bericht. Sie werden automatisch in kleinen Zeitabschnitten verarbeitet.</td></tr><?php endif; ?><?php foreach ($cronPendingJobs as $job): $jobTypeLabels = ['pdf_zip' => 'PDF-ZIP-Export', 'pdf_bundle' => 'Sammel-PDF', 'directory_import' => 'Datenimport', 'phoenix_sync' => 'Phoenix-Import']; $stateLabels = ['queued' => 'Wartet', 'running' => 'Läuft', 'done' => 'Fertig', 'error' => 'Fehler', 'cancelled' => 'Abgebrochen']; $jobType = $jobTypeLabels[(string) ($job['type'] ?? '')] ?? 'Hintergrundaufgabe'; $state = (string) ($job['state'] ?? ''); $step = (int) ($job['step'] ?? 0); $total = (int) ($job['total'] ?? 0); ?><tr><td class="text-break"><span class="fw-semibold"><?= htmlspecialchars($jobType) ?></span><br><span class="small text-body-secondary">Aufgabe <?= htmlspecialchars(substr((string) ($job['id'] ?? ''), 0, 12)) ?></span></td><td><span class="badge text-bg-<?= $state === 'running' ? 'primary' : 'secondary' ?>"><?= htmlspecialchars($stateLabels[$state] ?? 'Unbekannt') ?></span></td><td><?= $total > 0 ? htmlspecialchars($step . ' von ' . $total) : '—' ?></td><td><?= htmlspecialchars((string) ($job['current_device'] ?? '—')) ?></td><td class="text-break"><?= htmlspecialchars((string) ($job['message'] ?? 'Wartet auf den nächsten Hintergrundlauf.')) ?></td></tr><?php endforeach; ?></tbody></table></div></div><?php else: ?><p class="small text-body-secondary"><i class="fa-solid fa-check me-1" aria-hidden="true"></i>Keine ausstehenden Hintergrundaufgaben.</p><?php endif; ?>
