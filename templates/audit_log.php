...
<?php $formatJobDate = static function ($value): string { try { return (new DateTimeImmutable((string) $value))->setTimezone(new DateTimeZone('Europe/Berlin'))->format('d.m.Y H:i:s'); } catch (Throwable) { return '—'; } }; ?>
...
<?php if ($recentJobs !== []): ?><details class="mt-3"><summary class="small fw-semibold"><i class="fa-solid fa-clock-rotate-left me-1" aria-hidden="true"></i>Letzte Hintergrundaufgaben (<?= count($recentJobs) ?>)</summary><div class="table-responsive mt-2"><table class="table table-sm align-middle mb-0"><thead><tr><th>Aufgabe</th><th>Status</th><th>Zeitpunkt</th><th>Fortschritt</th><th>Meldung</th></tr></thead><tbody><?php foreach ($recentJobs as $job): $state = (string) ($job['state'] ?? ''); $total = (int) ($job['total'] ?? 0); $step = (int) ($job['step'] ?? 0); $stateClass = $state === 'done' ? 'success' : ($state === 'cancelled' ? 'secondary' : 'danger'); $stateLabel = ['done' => 'Fertig', 'cancelled' => 'Abgebrochen', 'error' => 'Fehlgeschlagen'][$state] ?? $state; $createdAt = (string) ($job['created_at'] ?? ''); $changedAt = (string) ($job['updated_at'] ?? $job['finished_at'] ?? ''); ?><tr><td><span class="fw-semibold"><?= htmlspecialchars((string) ($job['label'] ?? 'Hintergrundaufgabe')) ?></span><br><span class="small text-body-secondary font-monospace"><?= htmlspecialchars(substr((string) ($job['id'] ?? ''), 0, 12)) ?></span></td><td><span class="badge text-bg-<?= $stateClass ?>"><?= htmlspecialchars($stateLabel) ?></span></td><td class="text-nowrap"><?= htmlspecialchars($formatJobDate($changedAt !== '' ? $changedAt : $createdAt)) ?><?php if ($createdAt !== '' && $changedAt !== '' && $createdAt !== $changedAt): ?><br><span class="small text-body-secondary">angelegt <?= htmlspecialchars($formatJobDate($createdAt)) ?></span><?php endif; ?></td><td><?= $total > 0 ? htmlspecialchars($step . ' von ' . $total) : '—' ?></td><td class="text-break"><?= htmlspecialchars((string) ($job['message'] ?? '—')) ?></td></tr><?php endforeach; ?></tbody></table></div></details><?php endif; ?>
</div>
</section>
