  ...
  <div class="card-body">
    <?php foreach ($whatsNewEntries as $entryIndex => $entry): $done = !empty($checked[$entry['id'] ?? '']); ?>
      <article class="<?= $done ? 'bg-body-tertiary text-body-secondary' : 'border border-warning border-2 bg-warning-subtle' ?> <?= $entryIndex !== array_key_last((array) $whatsNewEntries) ? 'mb-3' : 'mb-0' ?> p-3 rounded">
        ...
        <?php if (!empty($entry['pruefapp_revision']) || !empty($entry['base_revision'])): ?><div class="d-flex flex-wrap gap-1 mt-2"><span class="badge text-bg-secondary">Prüfapp <?= htmlspecialchars((string) ($entry['pruefapp_revision'] ?? '—')) ?></span><span class="badge text-bg-light border text-body">Base <?= htmlspecialchars((string) ($entry['base_revision'] ?? '—')) ?></span></div><?php endif; ?>
