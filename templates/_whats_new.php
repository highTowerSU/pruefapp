  ...
  <div class="card-body">
    <?php foreach ($whatsNewEntries as $entryIndex => $entry): $done = !empty($checked[$entry['id'] ?? '']); ?>
      <article class="<?= $done ? 'bg-body-tertiary text-body-secondary' : 'border border-warning border-2 bg-warning-subtle' ?> <?= $entryIndex !== array_key_last((array) $whatsNewEntries) ? 'mb-3' : 'mb-0' ?> p-3 rounded">
        ...
        <?php if (!empty($entry['pruefapp_revision']) || !empty($entry['base_revision'])): ?><div class="d-flex flex-wrap gap-1 mt-2"><span class="badge text-bg-secondary">Prüfapp <?= htmlspecialchars((string) ($entry['pruefapp_revision'] ?? '—')) ?></span><span class="badge text-bg-secondary">Base <?= htmlspecialchars((string) ($entry['base_revision'] ?? '—')) ?></span></div><?php endif; ?>
        <ul class="mb-2 mt-2"><?php foreach ((array) $entry['items'] as $item): ?><li><?= htmlspecialchars((string) $item) ?></li><?php endforeach; ?></ul>
        <?php if ($done): ?><span class="small"><i class="fa-solid fa-check-double me-1" aria-hidden="true"></i>Gelesen</span><?php else: ?><button class="btn btn-sm btn-secondary" type="button" hx-post="<?= htmlspecialchars(url_for('downloads/was-ist-neu/' . rawurlencode((string) $entry['id']) . '/erledigt'), ENT_QUOTES) ?>" hx-target="#whats-new" hx-swap="outerHTML"><i class="fa-solid fa-check me-1" aria-hidden="true"></i>Gelesen</button><?php endif; ?>
      </article>
    <?php endforeach; ?>
  </div>
</section>
