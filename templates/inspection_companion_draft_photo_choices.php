...
$choiceAttribute = ($picker ?? '') === 'upload' ? 'data-media-upload-photo-choose' : 'data-companion-draft-photo-choose';
?>
<?php if ($pending === [] && $used === []): ?>
...
<?php else: ?>
  ...
  <?php foreach ($pending as $item): ?>
    <button type="button" class="btn btn-sm btn-primary text-start d-flex align-items-center gap-2" <?= $choiceAttribute ?>="<?= (int) $item['id'] ?>"><img class="rounded border bg-light" style="width:42px;height:32px;object-fit:cover" src="<?= htmlspecialchars($photoUrl($item), ENT_QUOTES) ?>" alt="Vorschau"><span><i class="fa-solid fa-camera me-1" aria-hidden="true"></i><?= htmlspecialchars($labels[(string) ($item['media_type'] ?? '')] ?? 'Foto') ?><?= trim((string) ($item['caption'] ?? '')) !== '' ? ' · ' . htmlspecialchars((string) $item['caption']) : '' ?></span></button>
  <?php endforeach; ?>
  ...
  <?php foreach (array_slice($used, 0, 12) as $item): ?>
    <button type="button" class="btn btn-sm btn-secondary text-start d-flex align-items-center gap-2" <?= $choiceAttribute ?>="<?= (int) $item['id'] ?>"><img class="rounded border bg-light" style="width:42px;height:32px;object-fit:cover" src="<?= htmlspecialchars($photoUrl($item), ENT_QUOTES) ?>" alt="Vorschau"><span><i class="fa-solid fa-clock-rotate-left me-1" aria-hidden="true"></i><?= htmlspecialchars($labels[(string) ($item['media_type'] ?? '')] ?? 'Foto') ?><?= trim((string) ($item['caption'] ?? '')) !== '' ? ' · ' . htmlspecialchars((string) $item['caption']) : '' ?></span></button>
  <?php endforeach; ?>
  </div>
<?php endif; ?>
