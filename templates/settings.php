...
<form id="settings-general-panel" data-action-nav="Einstellungen speichern" data-action-icon="fa-gear" method="post" action="<?= htmlspecialchars(url_for('admin/konfiguration'), ENT_QUOTES) ?>" class="card shadow-sm mb-4">
  ...
  <div class="card-footer text-end"><button type="submit" class="btn btn-primary"><i class="fa-solid fa-floppy-disk me-1" aria-hidden="true"></i>Speichern</button></div>
</form>

<?= render_template('settings_ai_provider.php', ['providers' => AiProviderService::all(), 'provider' => AiProviderService::selectedVocabularyProvider() ?? (object) ['id'=>0,'name'=>'','base_url'=>'','header_name'=>'Authorization','auth_mode'=>'token','api_token'=>'','pricing_url'=>'','oauth_authorization_url'=>'','oauth_token_url'=>'','oauth_client_id'=>'','oauth_client_secret'=>'','oauth_scopes'=>'','oauth_access_token'=>''], 'models' => ($providerForSettings = AiProviderService::selectedVocabularyProvider()) ? AiProviderService::models($providerForSettings) : [], 'enabled' => get_app_config('vocabulary_ai_enabled', '0') === '1', 'selectedProviderId' => (int) get_app_config('vocabulary_ai_provider_id', '0'), 'selectedModel' => (string) get_app_config('vocabulary_ai_model', ''), 'message' => '', 'error' => '']) ?>
    ...
    <div class="row g-3 mt-1"><div class="col-md-3"><label class="form-label" for="vocabulary_ai_auth_mode">Anmeldung</label><select class="form-select" id="vocabulary_ai_auth_mode" name="vocabulary_ai_auth_mode"><option value="token"<?= ($values['vocabulary_ai_auth_mode'] ?? '') === 'token' ? ' selected' : '' ?>>Token / API-Key</option><option value="oauth"<?= ($values['vocabulary_ai_auth_mode'] ?? '') === 'oauth' ? ' selected' : '' ?>>OAuth 2.0 mit PKCE</option></select></div><div class="col-md-5"><label class="form-label" for="vocabulary_ai_oauth_authorization_url">OAuth-Autorisierungs-URL</label><input class="form-control<?= isset($errors['vocabulary_ai_oauth_authorization_url']) ? ' is-invalid' : '' ?>" type="url" id="vocabulary_ai_oauth_authorization_url" name="vocabulary_ai_oauth_authorization_url" value="<?= htmlspecialchars($values['vocabulary_ai_oauth_authorization_url'] ?? '', ENT_QUOTES) ?>"></div><div class="col-md-4"><label class="form-label" for="vocabulary_ai_oauth_token_url">OAuth-Token-URL</label><input class="form-control<?= isset($errors['vocabulary_ai_oauth_token_url']) ? ' is-invalid' : '' ?>" type="url" id="vocabulary_ai_oauth_token_url" name="vocabulary_ai_oauth_token_url" value="<?= htmlspecialchars($values['vocabulary_ai_oauth_token_url'] ?? '', ENT_QUOTES) ?>"></div><div class="col-md-4"><label class="form-label" for="vocabulary_ai_oauth_client_id">OAuth-Client-ID</label><input class="form-control<?= isset($errors['vocabulary_ai_oauth_client_id']) ? ' is-invalid' : '' ?>" id="vocabulary_ai_oauth_client_id" name="vocabulary_ai_oauth_client_id" value="<?= htmlspecialchars($values['vocabulary_ai_oauth_client_id'] ?? '', ENT_QUOTES) ?>"></div><div class="col-md-4"><label class="form-label" for="vocabulary_ai_oauth_client_secret">Client-Secret<?= !empty($vocabularyAiOAuthSecretConfigured) ? ' (gesetzt)' : '' ?></label><input class="form-control" type="password" id="vocabulary_ai_oauth_client_secret" name="vocabulary_ai_oauth_client_secret" autocomplete="new-password" placeholder="optional / unverändert lassen"></div><div class="col-md-4"><label class="form-label" for="vocabulary_ai_oauth_scopes">Scopes</label><input class="form-control" id="vocabulary_ai_oauth_scopes" name="vocabulary_ai_oauth_scopes" value="<?= htmlspecialchars($values['vocabulary_ai_oauth_scopes'] ?? '', ENT_QUOTES) ?>" placeholder="z. B. openid profile"></div></div>
  </div>
...
<details class="card shadow-sm mt-4" id="settings-ai-panel" data-action-nav="KI-Stammdatenprüfung" data-action-icon="fa-wand-magic-sparkles">
  ...
  <div class="card-body">
    ...
    <hr>
    <div class="d-flex flex-wrap align-items-center gap-2"><form method="post" action="<?= htmlspecialchars(url_for('admin/konfiguration'), ENT_QUOTES) ?>"><input type="hidden" name="action" value="begin_vocabulary_oauth"><button class="btn btn-outline-success" type="submit"><i class="fa-solid fa-right-to-bracket me-1" aria-hidden="true"></i>OAuth verbinden</button></form><span class="small text-body-secondary"><?= !empty($vocabularyAiOAuthConnected) ? 'OAuth verbunden; Zugriffstoken wird automatisch erneuert.' : 'Vorher OAuth-Felder speichern; die Callback-URL beim Anbieter hinterlegen.' ?></span></div>
    <div class="form-text mt-2">Callback-URL: <code><?= htmlspecialchars(VocabularyOAuthService::callbackUrl(), ENT_QUOTES) ?></code></div>
  </div>
</details>
    <?php endif; ?>
