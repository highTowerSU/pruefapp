...
<?php if ((int) ($_GET['device_id'] ?? 0) > 0): ?><script>document.addEventListener('DOMContentLoaded',()=>{const device=document.getElementById('geraet-<?= (int) $_GET['device_id'] ?>');if(!device)return;device.open=true;requestAnimationFrame(()=>device.scrollIntoView({behavior:'smooth',block:'start'}));});</script><?php endif; ?>
