...
<?php endif; ?>
    <?php if ($canManage): ?><details id="device-new-panel" data-action-nav="Neues Gerät" data-action-icon="fa-plus" class="card device-card mb-4"><summary class="card-header device-card-summary d-flex align-items-center gap-2"><i class="fa-solid fa-plus-circle text-primary" aria-hidden="true"></i><strong>Neues Gerät</strong><span class="small text-body-secondary device-new-caption">Gerätedaten anlegen</span><a class="btn btn-sm btn-secondary ms-auto" href="<?= htmlspecialchars(url_for('profil') . '#companion-sessions', ENT_QUOTES) ?>" data-companion-connect-info data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="Smartphone für Barcode-Scan und Fotos koppeln. Öffnet den Kopplungsbereich im eigenen Profil."><i class="fa-solid fa-mobile-screen-button me-1" aria-hidden="true"></i>Companion-App verbinden</a></summary><div class="card-body"><?php $form(null, (string) ($newNumber ?? '')); ?></div></details><?php endif; ?>
<style>
