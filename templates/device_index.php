...
?>
  <form method="post" action="<?= htmlspecialchars(url_for('geraete'), ENT_QUOTES) ?>" enctype="multipart/form-data" class="row g-3 device-form">
    ...
    <div class="col-lg-4"><div class="d-flex justify-content-between align-items-center gap-1"><label class="form-label" for="device-name-<?= $formKey ?>"><i class="fa-solid fa-plug icon-slot me-1" aria-hidden="true"></i>Gerätebezeichnung</label><button class="btn btn-sm btn-secondary" type="button" data-companion-for="device-name-<?= $formKey ?>" title="Companion-Wert auswählen"><i class="fa-solid fa-mobile-screen-button" aria-hidden="true"></i></button></div><input class="form-control" id="device-name-<?= $formKey ?>" name="name" required value="<?= htmlspecialchars((string) $device->name) ?>"><div class="d-flex flex-wrap align-items-center gap-2 mt-2"><button type="button" class="btn btn-sm btn-primary" data-copy-device-name disabled><i class="fa-solid fa-wand-magic-sparkles me-1" aria-hidden="true"></i><span data-copy-device-name-label>Passende Bezeichnung</span></button><?php if ($formKey === 0): ?><div class="btn-group btn-group-sm" role="group" aria-label="Typenschild erfassen"><button type="button" class="btn btn-secondary" data-open-typeplate><i class="fa-solid fa-id-card me-1" aria-hidden="true"></i>Typenschild-KI erfassen</button><a class="btn btn-secondary" href="<?= htmlspecialchars(url_for('profil') . '#companion-sessions', ENT_QUOTES) ?>" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="Typenschild mit dem verbundenen Smartphone fotografieren oder zuerst die Companion-App koppeln."><i class="fa-solid fa-mobile-screen-button" aria-hidden="true"></i><span class="visually-hidden">Companion-App öffnen</span></a></div><?php endif; ?></div><?php if ($formKey === 0): ?><div class="mt-2" data-draft-photo-result aria-live="polite"></div><?php endif; ?><div class="form-text" data-device-name-hint>Nach der Modellwahl erscheint hier ein eindeutiger Vorschlag.</div></div>
...
(() => {
  ...
  const applyTypeplateProposal = (form, proposal) => { ['manufacturer','device_model','name','serial_number','inventory_number'].forEach(key => { const value = String((proposal && proposal[key]) || '').trim(); const field = form.querySelector(`[name="${key}"]`); if (!value || !field) return; if (field.tomselect) field.tomselect.setValue(value, true); else { field.value = value; field.dispatchEvent(new Event('change', {bubbles:true})); } }); };
