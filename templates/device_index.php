...
?>
  <form method="post" action="<?= htmlspecialchars(url_for('geraete'), ENT_QUOTES) ?>" enctype="multipart/form-data" class="row g-3 device-form">
    ...
    <?php if ($formKey === 0): ?><div class="col-12"><details class="border rounded-3 bg-body-tertiary" data-typeplate-panel><summary class="p-3 fw-semibold"><i class="fa-solid fa-camera text-primary me-2" aria-hidden="true"></i>Fotos zum neuen Gerät <span class="small text-body-secondary fw-normal">optional</span></summary><div class="p-3 pt-0"><p class="small text-body-secondary mb-2">Typenschild, Zustand oder Mangel fotografieren. Das Foto bleibt nach dem Vorab-Upload bis zum Speichern diesem neuen Gerät zugeordnet. Bei „Typenschild“ erzeugt die KI direkt einen übernehmbaren Vorschlag.</p><input type="hidden" name="new_device_draft_token" value="" data-draft-photo-token><div class="row g-2"><div class="col-md-5"><label class="form-label" for="new-device-photo">Foto</label><input class="form-control" id="new-device-photo" name="new_device_photo" type="file" accept="image/jpeg,image/png,image/webp" capture="environment" data-stage-device-photo></div><div class="col-md-3"><label class="form-label" for="new-device-photo-type">Fotoart</label><select class="form-select" id="new-device-photo-type" name="new_device_photo_type"><option value="type_plate">Typenschild · KI-Vorschlag</option><option value="condition">Zustand</option><option value="defect">Mangel</option><option value="disposal">Aussonderung</option><option value="other">Sonstiges</option></select></div><div class="col-md-4"><label class="form-label" for="new-device-photo-caption">Bemerkung</label><input class="form-control" id="new-device-photo-caption" name="new_device_photo_caption" maxlength="1000" placeholder="optional"></div></div><div class="mt-3"><button class="btn btn-primary" type="button" data-stage-device-photo-upload><i class="fa-solid fa-upload me-1" aria-hidden="true"></i>Foto hochladen<?= $formKey === 0 ? ' und Vorschlag erzeugen' : '' ?></button></div><div class="mt-2" data-draft-photo-result aria-live="polite"></div></div></details></div><?php endif; ?>
    <div class="col-md-6"><details class="border rounded-3"><summary class="p-2 fw-semibold"><i class="fa-solid fa-sliders icon-slot me-1" aria-hidden="true"></i>Zusatzattribute <span class="small text-body-secondary fw-normal">optional</span></summary><div class="p-2 pt-0"><div class="metadata-visual-editor" data-metadata-editor><div class="metadata-visual-rows d-grid gap-2"></div><button class="btn btn-sm btn-secondary mt-2" type="button" data-metadata-add><i class="fa-solid fa-plus me-1" aria-hidden="true"></i>Attribut hinzufügen</button><details class="mt-2"><summary class="small text-body-secondary">JSON-Ansicht für Fortgeschrittene</summary><textarea class="form-control font-monospace mt-2" id="device-metadata-<?= $formKey ?>" name="metadata_json" data-metadata-json placeholder='z. B. {"kostenstelle":"1000"}'><?= htmlspecialchars($metadataValue) ?></textarea></details></div><div class="form-text">Text, Zahl, Datum, Ja/Nein oder MAC-Adresse; die Daten werden weiterhin als kompatibles JSON gespeichert.</div></div></details></div>
    <?php $preferred = null; foreach ($inspectionTypes as $inspectionType) if ((string) $inspectionType['code'] === $preferredInspectionType) { $preferred = $inspectionType; break; } $preferred ??= $inspectionTypes[0] ?? ['code' => 'electrical', 'name' => 'Elektroprüfung', 'icon' => 'fa-bolt']; ?>
    <div class="col-12 text-end d-flex justify-content-end gap-2"><button class="btn btn-secondary btn-sm" name="save_only" value="1"><i class="fa-solid fa-floppy-disk me-1" aria-hidden="true"></i>Speichern</button><?php if ($formKey > 0): ?><div class="btn-group"><?php if (!empty($preferred['allowed'])): ?><a class="btn btn-primary btn-sm" href="<?= htmlspecialchars(url_for('geraete/' . $formKey . '/pruefungen/neu?inspection_type_code=' . rawurlencode((string) $preferred['code'])), ENT_QUOTES) ?>"><i class="fa-solid <?= htmlspecialchars((string) ($preferred['icon'] ?: 'fa-clipboard-check'), ENT_QUOTES) ?> me-1" aria-hidden="true"></i>Neue <?= htmlspecialchars((string) $preferred['name']) ?></a><?php else: ?><button class="btn btn-primary btn-sm" type="button" disabled title="<?= htmlspecialchars((string) ($preferred['permission_message'] ?? ''), ENT_QUOTES) ?>"><i class="fa-solid <?= htmlspecialchars((string) ($preferred['icon'] ?: 'fa-clipboard-check'), ENT_QUOTES) ?> me-1" aria-hidden="true"></i>Neue <?= htmlspecialchars((string) $preferred['name']) ?></button><?php endif; ?><button type="button" class="btn btn-primary btn-sm dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-expanded="false"><span class="visually-hidden">Andere Prüfart auswählen</span></button><ul class="dropdown-menu dropdown-menu-end"><?php foreach ($inspectionTypes as $inspectionType): if ((string) $inspectionType['code'] === (string) $preferred['code']) continue; ?><li><?php if (!empty($inspectionType['allowed'])): ?><a class="dropdown-item" href="<?= htmlspecialchars(url_for('geraete/' . $formKey . '/pruefungen/neu?inspection_type_code=' . rawurlencode((string) $inspectionType['code'])), ENT_QUOTES) ?>"><i class="fa-solid <?= htmlspecialchars((string) ($inspectionType['icon'] ?: 'fa-clipboard-check'), ENT_QUOTES) ?> icon-slot me-2" aria-hidden="true"></i>Neue <?= htmlspecialchars((string) $inspectionType['name']) ?></a><?php else: ?><span class="dropdown-item disabled" title="<?= htmlspecialchars((string) ($inspectionType['permission_message'] ?? ''), ENT_QUOTES) ?>"><i class="fa-solid <?= htmlspecialchars((string) ($inspectionType['icon'] ?: 'fa-clipboard-check'), ENT_QUOTES) ?> icon-slot me-2" aria-hidden="true"></i>Neue <?= htmlspecialchars((string) $inspectionType['name']) ?><small class="d-block ms-4 text-wrap"><?= htmlspecialchars((string) ($inspectionType['permission_message'] ?? 'Noch nicht berechtigt.')) ?></small></span><?php endif; ?></li><?php endforeach; ?></ul></div><?php else: ?><button class="btn btn-primary btn-sm" name="save_and_inspect" value="1"><i class="fa-solid fa-clipboard-check me-1" aria-hidden="true"></i>Speichern und neue Prüfung</button><?php endif; ?></div>
  </form>
<?php }; ?>
...
(() => {
  ...
  const applyTypeplateProposal = (form, proposal) => { ['manufacturer','device_model','name','serial_number','inventory_number'].forEach(key => { const value = String(proposal?.[key] || '').trim(); const field = form.querySelector(`[name="${key}"]`); if (!value || !field) return; if (field.tomselect) field.tomselect.setValue(value, true); else { field.value = value; field.dispatchEvent(new Event('change', {bubbles:true})); } }); };
  const stageNewDevicePhotos = () => document.querySelectorAll('form.device-form').forEach(form => { const photo = form.querySelector('[data-stage-device-photo]'); if (!photo || photo.dataset.bound) return; photo.dataset.bound = '1'; photo.addEventListener('change', async () => { if (photo.dataset.stageRequested !== '1' || !photo.files?.[0]) return; delete photo.dataset.stageRequested; const result = form.querySelector('[data-draft-photo-result]'); const token = form.querySelector('[data-draft-photo-token]'); const type = form.querySelector('[name="new_device_photo_type"]'); const caption = form.querySelector('[name="new_device_photo_caption"]'); result.innerHTML = '<div class="alert alert-info py-2 mb-0"><span class="spinner-border spinner-border-sm me-1" aria-hidden="true"></span>Foto wird hochgeladen' + (type?.value === 'type_plate' ? ' und Typenschild ausgewertet' : '') + ' …</div>'; const body = new FormData(); body.append('photo', photo.files[0]); body.append('media_type', type?.value || 'condition'); body.append('caption', caption?.value || ''); try { const response = await fetch(<?= json_encode(url_for('geraete/fotos/vorlaeufig'), JSON_UNESCAPED_UNICODE) ?>, {method: 'POST', body, headers: {'Accept': 'application/json'}}); const data = await response.json(); if (!response.ok || !data.ok) throw new Error(data.error || 'Foto konnte nicht hochgeladen werden.'); token.value = data.token || ''; const proposal = data.proposal || null; const fields = proposal ? ['manufacturer','device_model','name','serial_number','inventory_number'].filter(key => String(proposal[key] || '').trim()).map(key => `<li><strong>${key === 'name' ? 'Bezeichnung' : key === 'device_model' ? 'Modell' : key === 'manufacturer' ? 'Hersteller' : key === 'serial_number' ? 'Seriennummer' : 'Inventar'}:</strong> ${String(proposal[key]).replace(/[&<>]/g, char => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[char]))}</li>`).join('') : ''; const analysisNote = data.analysis_error ? `<div class="small mt-1">KI-Vorschlag nicht verfügbar: ${String(data.analysis_error).replace(/[&<>]/g, char => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[char]))}</div>` : ''; result.innerHTML = proposal ? `<div class="alert alert-success py-2 mb-0"><strong><i class="fa-solid fa-wand-magic-sparkles me-1" aria-hidden="true"></i>Typenschildvorschlag bereit</strong><ul class="mb-2 mt-1">${fields}</ul><button class="btn btn-sm btn-primary" type="button" data-apply-staged-typeplate><i class="fa-solid fa-arrow-down me-1" aria-hidden="true"></i>Vorschlag übernehmen</button></div>` : `<div class="alert ${data.analysis_error ? 'alert-warning' : 'alert-success'} py-2 mb-0"><i class="fa-solid fa-check me-1" aria-hidden="true"></i>Foto ist hochgeladen und wird beim Speichern dem Gerät zugeordnet.${analysisNote}</div>`; if (proposal) result.querySelector('[data-apply-staged-typeplate]')?.addEventListener('click', () => applyTypeplateProposal(form, proposal)); } catch (error) { result.innerHTML = `<div class="alert alert-danger py-2 mb-0">${String(error.message || error)}</div>`; } }); });
  window.addEventListener('DOMContentLoaded', stageNewDevicePhotos, {once: true});
  document.addEventListener('click', event => { const button = event.target.closest('[data-stage-device-photo-upload]'); if (!button) return; const form = button.closest('form.device-form'); const photo = form?.querySelector('[data-stage-device-photo]'); if (!photo) return; if (!photo.files?.[0]) { form?.querySelector('[data-typeplate-panel]')?.setAttribute('open', ''); photo.click(); return; } photo.dataset.stageRequested = '1'; photo.dispatchEvent(new Event('change', {bubbles: true})); });
  document.addEventListener('click', event => { const button = event.target.closest('[data-open-typeplate]'); if (!button) return; const form = button.closest('form.device-form'); const panel = form?.querySelector('[data-typeplate-panel]'); const photo = panel?.querySelector('[data-stage-device-photo]'); if (!panel || !photo) return; panel.open = true; panel.querySelector('[name="new_device_photo_type"]')?.value = 'type_plate'; const result = panel.querySelector('[data-draft-photo-result]'); if (result) result.innerHTML = '<div class="alert alert-info py-2 mb-0">Typenschildfoto auswählen – danach wird es direkt hochgeladen und ausgewertet.</div>'; photo.dataset.stageRequested = '1'; photo.click(); });
...
<?php if (!empty($canManage) || !empty($canBulkManage)): ?><details id="device-actions-panel" data-action-nav="Geräteaktionen" data-action-icon="fa-list-check" class="card mb-3"><summary class="card-header fw-semibold">Auswahl &amp; Massenaktionen</summary><div class="card-body">
