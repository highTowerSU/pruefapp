...
$isRegiePosition = static fn(array $position): bool => (string) ($position['kind'] ?? '') === 'regie' || ((string) ($position['kind'] ?? '') === 'unclassified' && (string) ($position['suggested_kind'] ?? '') === 'regie');
...
<section class="card mb-4" id="billing-reconciliation">
  ...
  <div class="card-body">
    <div class="row g-3">
      <div class="col-sm-6 col-lg"><strong>Geräte laut Rechnung</strong><div><?= htmlspecialchars((string) ($reconciliation['deviceTarget'] ?? 0)) ?></div></div>
      <div class="col-sm-6 col-lg"><strong>Geräte zugeordnet</strong><div><?= htmlspecialchars((string) ($reconciliation['deviceActual'] ?? 0)) ?></div></div>
      <div class="col-sm-6 col-lg"><strong>Regie laut SevDesk</strong><div><?= (int) ($reconciliation['nativeRegieTarget'] ?? $reconciliation['regieTarget'] ?? 0) ?> Minuten</div></div>
      <div class="col-sm-6 col-lg"><strong>Regie nach Umbuchungen</strong><div><?= (int) ($reconciliation['regieTarget'] ?? 0) ?> Minuten</div></div>
      <div class="col-sm-6 col-lg"><strong>Regie zugeordnet</strong><div><?= (int) ($reconciliation['regieActual'] ?? 0) ?> Minuten</div></div>
    </div>
    <?php if ((int) ($reconciliation['regieTransferredOut'] ?? 0) > 0 || (int) ($reconciliation['regieTransferredIn'] ?? 0) > 0): ?><div class="alert alert-info py-2 mt-3 mb-0"><i class="fa-solid fa-arrow-right-arrow-left me-1" aria-hidden="true"></i>Regie-Umbuchungen: <?= (int) ($reconciliation['regieTransferredOut'] ?? 0) ?> Min. übertragen · <?= (int) ($reconciliation['regieTransferredIn'] ?? 0) ?> Min. übernommen.</div><?php endif; ?>
    ...
    <?php if (($reconciliation['positions'] ?? []) !== []): ?>
      ...
      <?php foreach ($reconciliation['positions'] as $position): $storedKind = (string) $position['kind']; $suggestedKind = (string) $position['suggested_kind']; $selectedKind = in_array($storedKind, ['device', 'regie', 'other'], true) ? $storedKind : (in_array($suggestedKind, ['device', 'regie', 'other'], true) ? $suggestedKind : ''); ?>
        <tr><td><?= htmlspecialchars((string) $position['position_number']) ?></td><td><div class="fw-semibold"><?= htmlspecialchars((string) $position['name']) ?></div><?php if (trim((string) ($position['details'] ?? '')) !== ''): ?><div class="small text-body-secondary text-break mt-1"><?= nl2br(htmlspecialchars((string) $position['details'])) ?></div><?php endif; ?></td><td><?= htmlspecialchars((string) $position['quantity']) ?></td><td><?= htmlspecialchars(BillingController::displaySevDeskUnit((string) $position['unit'])) ?></td><td>
    ...
    <?php endif; ?>
    <?php $sourceRegiePositions = array_values(array_filter($reconciliation['positions'] ?? [], $isRegiePosition)); ?>
    <?php if (current_user_is_superadmin() && $sourceRegiePositions !== []): ?><details class="border rounded-3 p-3 mt-3" data-action-nav="Regie umbuchen" data-action-icon="fa-arrow-right-arrow-left"><summary class="fw-semibold"><i class="fa-solid fa-arrow-right-arrow-left me-1" aria-hidden="true"></i>Regiezeit auf andere Rechnung umbuchen</summary><div class="pt-3"><p class="small text-body-secondary">Die originale SevDesk-Position bleibt unverändert. Die Umbuchung verschiebt nur die geprüfte Regiezuordnung zwischen Rechnungen desselben Kunden und wird im Abgleich beider Rechnungen sichtbar.</p><?php if ($transferTargets === []): ?><div class="alert alert-warning py-2 mb-0">Für diesen Kunden gibt es keine andere aktive Regieposition als Ziel.</div><?php else: ?><form method="post" action="<?= htmlspecialchars(url_for('admin/abrechnung/rechnung/' . (int) $invoice->id . '/regie-uebertragen'), ENT_QUOTES) ?>" hx-post="<?= htmlspecialchars(url_for('admin/abrechnung/rechnung/' . (int) $invoice->id . '/regie-uebertragen'), ENT_QUOTES) ?>" hx-target="#billing-reconciliation" hx-select="#billing-reconciliation" hx-swap="outerHTML" class="row g-3 align-items-end"><div class="col-md-3"><label class="form-label">Quellposition</label><select class="form-select" name="source_position_id" required><?php foreach ($sourceRegiePositions as $position): ?><option value="<?= (int) $position['id'] ?>"><?= htmlspecialchars((string) $position['position_number'] . ' · ' . $position['name'] . ' (' . (int) $position['regie_minutes'] . ' Min.)') ?></option><?php endforeach; ?></select></div><div class="col-md-4"><label class="form-label">Zielposition</label><select class="form-select" name="target_position_id" required><option value="">Bitte wählen</option><?php foreach ($transferTargets as $target): ?><option value="<?= (int) $target['position_id'] ?>"><?= htmlspecialchars((string) ($target['invoice_number'] ?: '#' . $target['invoice_id']) . ' · ' . $target['position_number'] . ' · ' . $target['name']) ?></option><?php endforeach; ?></select></div><div class="col-md-2"><label class="form-label">Minuten</label><div class="input-group"><input class="form-control" type="number" name="minutes" min="1" required><span class="input-group-text">Min.</span></div></div><div class="col-md-3"><label class="form-label">Begründung</label><input class="form-control" name="reason" required placeholder="z. B. falscher Abrechnungslauf"></div><div class="col-md-8"><label class="form-check"><input class="form-check-input" type="checkbox" name="confirm" value="1" required> <span class="form-check-label">Ich habe Quelle, Ziel und Minuten geprüft.</span></label></div><div class="col-md-4"><button class="btn btn-warning w-100" type="submit"><i class="fa-solid fa-arrow-right-arrow-left me-1" aria-hidden="true"></i>Regie umbuchen</button></div></form><?php endif; ?><?php if ($transferRows !== []): ?><div class="table-responsive mt-3"><table class="table table-sm mb-0"><thead><tr><th>Zeitpunkt</th><th>Von</th><th>Nach</th><th>Minuten</th><th>Begründung</th></tr></thead><tbody><?php foreach ($transferRows as $transfer): ?><tr><td class="text-nowrap"><?= htmlspecialchars((string) $transfer['created_at']) ?></td><td><?= htmlspecialchars((string) ($transfer['source_invoice_number'] ?: '#' . $transfer['source_invoice_id']) . ' · ' . $transfer['source_position_number'] . ' · ' . $transfer['source_position_name']) ?></td><td><?= htmlspecialchars((string) ($transfer['target_invoice_number'] ?: '#' . $transfer['target_invoice_id']) . ' · ' . $transfer['target_position_number'] . ' · ' . $transfer['target_position_name']) ?></td><td><?= (int) $transfer['minutes'] ?> Min.</td><td><?= htmlspecialchars((string) $transfer['reason']) ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></div></details><?php endif; ?>
  </div>
</section>
...
<details class="card mb-4"<?= $suggestion['candidates'] !== [] ? ' open' : '' ?>><summary class="card-header fw-semibold"><i class="fa-solid fa-wand-magic-sparkles me-2" aria-hidden="true"></i>Zuordnungsvorschlag prüfen<?= $suggestion['candidates'] !== [] ? ' · ' . count($suggestion['candidates']) . ' Prüfungen' : '' ?></summary><div class="card-body">
  ...
  <?php if ($suggestion['candidates'] !== [] && $devicePositions !== []):
    $remainingRegie = max(0, (int) ($reconciliation['regieTarget'] ?? 0) - (int) ($reconciliation['regieActual'] ?? 0));
  ...
  <?php elseif ($suggestion['candidates'] !== []): ?><div class="alert alert-warning mb-0">Bitte oben mindestens eine Rechnungsposition als „Geräte“ bestätigen. Danach steht der Sammelvorschlag bereit.</div><?php endif; ?>
</div></details>
<?php endif; ?>
<?php if (current_user_is_superadmin() && ($reconciliation['positions'] ?? []) !== []): ?><details class="card mb-4"><summary class="card-header fw-semibold"><i class="fa-solid fa-link me-2" aria-hidden="true"></i>Historische Prüfung bewusst zuordnen</summary><div class="card-body"><p class="small text-body-secondary">Diese Bestätigung erzeugt eine nachvollziehbare Zuordnung. Nicht eindeutige Fälle bleiben bis dahin offen.</p><form method="post" action="<?= htmlspecialchars(url_for('admin/abrechnung/rechnung/' . (int) $invoice->id . '/historisch-zuordnen'), ENT_QUOTES) ?>" class="row g-3"><div class="col-md-5"><label class="form-label">Prüfung</label><select class="form-select" name="inspection_id" required><option value="">Bitte wählen</option><?php foreach (($candidates ?? []) as $candidate): ?><option value="<?= (int) $candidate['id'] ?>"><?= htmlspecialchars((string) $candidate['external_number'] . ' · ' . $candidate['test_date'] . ' · ' . $candidate['device_number']) ?></option><?php endforeach; ?></select></div><div class="col-md-3"><label class="form-label">Geräteposition</label><select class="form-select" name="device_position_id" required><option value="">Bitte wählen</option><?php foreach ($reconciliation['positions'] as $position): ?><option value="<?= (int) $position['id'] ?>"><?= htmlspecialchars((string) $position['position_number'] . ' · ' . $position['name']) ?></option><?php endforeach; ?></select></div><div class="col-md-2"><label class="form-label">Regieposition</label><select class="form-select" name="regie_position_id"><option value="">keine</option><?php foreach ($reconciliation['positions'] as $position): ?><option value="<?= (int) $position['id'] ?>"><?= htmlspecialchars((string) $position['position_number'] . ' · ' . $position['name']) ?></option><?php endforeach; ?></select></div><div class="col-md-1"><label class="form-label">Geräte</label><input class="form-control" type="number" name="quantity" min="0" value="1"></div><div class="col-md-1"><label class="form-label">Regie</label><input class="form-control" type="number" name="regie_minutes" min="0" value="0"></div><div class="col-12"><button class="btn btn-primary" type="submit"><i class="fa-solid fa-check me-1" aria-hidden="true"></i>Zuordnung bestätigen</button></div></form></div></details><?php endif; ?>
<?php if (current_user_is_superadmin() && (string) ($invoice->status ?? '') === 'sevdesk' && trim((string) ($invoice->sevdesk_invoice_id ?? '')) !== ''): ?><form method="post" action="<?= htmlspecialchars(url_for('admin/abrechnung/rechnung/' . (int) $invoice->id . '/sevdesk-entwurf-loeschen'), ENT_QUOTES) ?>" class="alert alert-warning d-flex flex-wrap justify-content-between align-items-center gap-2"><div><strong>Entwurf neu erstellen</strong><div class="small">Löscht ausschließlich den noch unveränderten Entwurf in SevDesk und gibt die verknüpften Prüfungen wieder frei. Die Exporthistorie bleibt erhalten.</div></div><input type="hidden" name="confirm" value="1"><button class="btn btn-danger" type="submit" onclick="return confirm('SevDesk-Entwurf wirklich löschen und alle Prüfungen wieder freigeben?');"><i class="fa-solid fa-trash me-1" aria-hidden="true"></i>Entwurf löschen &amp; neu abrechnen</button></form><?php endif; ?>
