...
<div class="col-12 d-flex gap-2 flex-wrap"><button class="btn btn-outline-primary" name="complete" value="0">Zwischenspeichern</button><button class="btn btn-primary" name="complete" value="1">Prüfung abschließen</button><?php if ((string) ($inspection->result_status ?? '') === 'ausstehend' && current_user_has_role('admin', 'editor')): ?><form method="post" action="<?= htmlspecialchars(url_for('admin/pruefungen/' . (int) $inspection->id . '/loeschen'), ENT_QUOTES) ?>" onsubmit="return confirm('Diese neue, noch ausstehende Prüfung wirklich löschen?');"><button class="btn btn-outline-danger" type="submit">Prüfung löschen</button></form><?php endif; ?></div>
</form></div></div>
