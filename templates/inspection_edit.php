...
<?php $resultStatus = trim((string) ($inspection->result_status ?? 'ausstehend')); $resultClass = $resultStatus === 'bestanden' ? 'success' : ($resultStatus === 'durchgefallen' ? 'danger' : 'warning'); $resultLabel = $resultStatus === 'bestanden' ? 'Bestanden' : ($resultStatus === 'durchgefallen' ? 'Durchgefallen' : 'Ausstehend'); $measurementValues = json_decode((string) ($inspection->measurements_json ?? ''), true); $measurementCount = is_array($measurementValues) ? count($measurementValues) : 0; ?>
<div class="alert alert-<?= $resultClass ?> d-flex flex-wrap align-items-center justify-content-between gap-2"><span><strong>Ergebnis: <?= htmlspecialchars($resultLabel) ?></strong><?php if ($resultStatus === 'bestanden' && $measurementCount === 0): ?> · Sicht-/Funktionsprüfung bestanden, aber noch keine Messwerte gespeichert.<?php elseif ($resultStatus === 'ausstehend'): ?> · Prüfung noch nicht abgeschlossen.<?php endif; ?></span><span class="badge text-bg-<?= $resultClass ?>"><?= $measurementCount ?> Messwerte</span></div>
<form method="post" class="row g-3">
<div class="col-md-6"><label class="form-label">Neue Gerätenummer / Etikett</label><input class="form-control" name="external_number" value="<?= htmlspecialchars((string) (preg_replace('/-(?:\d{2}|20\d{2})$/', '', (string) $inspection->external_number) ?: $inspection->external_number)) ?>" required><div class="form-text">Jahreszusatz wird automatisch aus dem Prüfdatum ergänzt. Derzeitige Gerätenummer: <?= htmlspecialchars((string) (($device->external_number ?? '') ?: 'nicht hinterlegt')) ?>.</div></div><div class="col-md-3"><label class="form-label">Prüfdatum</label><input class="form-control" type="date" name="test_date" value="<?= htmlspecialchars((string) $inspection->test_date) ?>" required><div class="form-text">Vorausgefüllt, bei Bedarf änderbar.</div></div><div class="w-100"></div>
...
<div class="col-md-3"><label class="form-label d-block">Abrechnung</label><div class="form-check form-switch mt-2"><input class="form-check-input" type="checkbox" role="switch" id="billable" name="billable" value="1"<?= !empty($inspection->billable) ? ' checked' : '' ?>><label class="form-check-label" for="billable">Abrechenbar</label></div><div class="form-text">Nur abrechenbare Prüfungen werden in der Abrechnung vorgeschlagen.</div></div>
...
<div class="col-12 d-flex gap-2 flex-wrap"><button class="btn btn-outline-primary" name="complete" value="0">Zwischenspeichern</button><button class="btn btn-primary" name="complete" value="1">Prüfung abschließen</button><?php if ((string) ($inspection->result_status ?? '') === 'ausstehend' && current_user_has_role('admin', 'editor')): ?><button class="btn btn-outline-danger" type="submit" data-double-confirm formaction="<?= htmlspecialchars(url_for('admin/pruefungen/' . (int) $inspection->id . '/loeschen'), ENT_QUOTES) ?>" formmethod="post">Prüfung löschen</button><?php endif; ?></div>
</form></div></div>
...
(() => {
  ...
  if (protectionGrid && !protectionGrid.querySelector('[value="Drehstrom"]')) {
    ...
    wrapper.innerHTML = '<label class="border rounded p-3 d-block protection-special-choice"><input class="visually-hidden" type="radio" name="protection_class" value="Drehstrom"<?= (string) $inspection->protection_class === 'Drehstrom' ? ' checked' : '' ?>><span class="d-flex align-items-center gap-3"><span class="sk-symbol" aria-hidden="true">⚡ 3~</span><span><strong>CEE-Drehstromkabel 400 V</strong><span class="d-block small text-body-secondary">BENNING ST 725 · 16 A / 32 A · fünfpolige CEE-Leitung</span></span><span class="badge text-bg-secondary ms-auto">Sonderfall</span></span><img class="img-fluid rounded mt-3 d-block cee-drehstrom-image" src="<?= htmlspecialchars(url_for('public/img/stecker/cee-drehstrom-16a-verlaengerung.jpg'), ENT_QUOTES) ?>" alt="Rote fünfpolige CEE-16-A-Verlängerungsleitung mit Stecker und Kupplung" loading="lazy"><span class="d-block small text-body-secondary mt-2">Prüfung mit einem passenden BENNING-CEE-Messadapter am ST 725; passive oder aktive Prüfung je nach Verfahren und Adapter.</span></label>';
  ...
  if (criteriaPanel && !document.getElementById('criteria-catalog')) {
    ...
    const renderCriteria = () => {
      ...
      criteriaPanel.innerHTML = criteria ? `<h2 class="h5 mb-2">${criteria.title} <span class="badge text-bg-${warmingDevice ? 'warning' : 'secondary'} ms-2">Wärmegerät: ${warmingDevice ? 'Ja' : 'Nein'}</span></h2><ul class="mb-0">${criteria.items.map(item => `<li>${item}</li>`).join('')}</ul>${selected === 'I' || selected === 'Kabel' ? '<div class="rsl-calculator mt-3"><label class="form-label" for="rsl-length">Leitungslänge (optional)</label><div class="input-group"><input class="form-control" id="rsl-length" name="cable_length_m" type="number" min="0" step="0.1" inputmode="decimal" value="<?= htmlspecialchars((string) ($inspection->cable_length_m ?? ''), ENT_QUOTES) ?>" placeholder="Leitungslänge in Metern"><span class="input-group-text">m</span><output class="form-control rsl-result" for="rsl-length">≤ 0,30 Ω</output></div><div class="form-text">Bis 5 m: 0,30 Ω; je weitere 7,5 m: +0,10 Ω; maximal 1,00 Ω. Der Grenzwert wird gespeichert und später gegen die Messung geprüft.</div></div>' : ''}` : '<h2 class="h5 mb-2">Kriterienkatalog</h2><p class="mb-0 text-body-secondary">Bitte zuerst eine Schutzklasse auswählen.</p>';
