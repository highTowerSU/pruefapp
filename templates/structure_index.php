...
$form = static function (string $type, $entity = null) use (
    ...
    $entity ??= (object) [
        ...
        'invoice_recipient_name' => '', 'invoice_contact_name' => '', 'invoice_address_street' => '', 'invoice_address_zip' => '', 'invoice_address_city' => '', 'invoice_address_country' => 'Deutschland', 'sevdesk_contact_address_id' => '',
    ];
    $config = [
...
<?php if ($type === 'customer'): ?><script>if(!window.pruefappSevdeskAddressChoiceBound){window.pruefappSevdeskAddressChoiceBound=true;document.addEventListener('change',function(event){const select=event.target.closest('[data-sevdesk-address-choice]');if(!select)return;const option=select.options[select.selectedIndex];const form=select.closest('form');if(!option||!form)return;const set=(name,value)=>{const field=form.querySelector('[name="'+name+'"]');if(field)field.value=value||'';};set('sevdesk_contact_address_id',option.value);set('invoice_recipient_name',option.dataset.name);set('invoice_contact_name',option.dataset.name2);set('invoice_address_street',option.dataset.street);set('invoice_address_zip',option.dataset.zip);set('invoice_address_city',option.dataset.city);set('invoice_address_country',option.dataset.country);});}</script><?php endif; ?>
<?php };
