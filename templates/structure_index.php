...
$form = static function (string $type, $entity = null) use (
    ...
    $entity ??= (object) [
        ...
        'invoice_recipient_name' => '', 'invoice_contact_name' => '', 'invoice_address_street' => '', 'invoice_address_zip' => '', 'invoice_address_city' => '', 'invoice_address_country' => 'Deutschland', 'sevdesk_contact_address_id' => '', 'sevdesk_contact_person_id' => '',
    ];
    $config = [
...
<form method="post" action="<?= htmlspecialchars(url_for($config['route']), ENT_QUOTES) ?>" class="row g-2">
  ...
  <?php endif; ?>
  <?php if ($type === 'customer'): ?><div class="col-md-6"><label class="form-label">Mandant</label><select class="form-select" name="tenant_id" required data-sevdesk-tenant><option value="">Mandant wählen</option><?php foreach ($tenants as $tenant): ?><option value="<?= (int) $tenant->id ?>"<?= (int) ($entity->tenant_id ?? 0) === (int) $tenant->id ? ' selected' : '' ?>><?= htmlspecialchars((string) $tenant->name) ?></option><?php endforeach; ?></select></div><div class="col-md-6"><label class="form-label">SevDesk-Kundennummer</label><?php $hasContacts = false; foreach ($sevdeskContactsByTenant as $contacts) if ($contacts !== []) { $hasContacts = true; break; } if ($hasContacts): ?><select class="form-select" name="sevdesk_customer_id" data-sevdesk-contact><option value="">Keine Zuordnung</option><?php foreach ($sevdeskContactsByTenant as $tenantId => $contacts): foreach ($contacts as $contact): $contactId = (string) ($contact['id'] ?? ''); $contactNumber = (string) ($contact['customerNumber'] ?? $contact['customer_number'] ?? ''); $contactName = trim((string) ($contact['name'] ?? (($contact['familyname'] ?? '') . ' ' . ($contact['firstname'] ?? '')))); if ($contactId === '') continue; ?><option value="<?= htmlspecialchars($contactId, ENT_QUOTES) ?>" data-tenant-id="<?= (int) $tenantId ?>" data-customer-number="<?= htmlspecialchars($contactNumber, ENT_QUOTES) ?>"<?= (string) ($entity->sevdesk_customer_id ?? '') === $contactId ? ' selected' : '' ?>><?= htmlspecialchars(($contactNumber !== '' ? $contactNumber . ' · ' : '') . ($contactName ?: 'SevDesk-Kunde ' . $contactId)) ?></option><?php endforeach; endforeach; ?></select><input type="hidden" name="sevdesk_customer_number" value="<?= htmlspecialchars((string) ($entity->sevdesk_customer_number ?? ''), ENT_QUOTES) ?>"><div class="form-text">Nur Kontakte des ausgewählten Mandanten.</div><?php else: ?><input class="form-control" name="sevdesk_customer_number" value="<?= htmlspecialchars((string) ($entity->sevdesk_customer_number ?? $entity->sevdesk_customer_id ?? '')) ?>" placeholder="z. B. 123456"><input type="hidden" name="sevdesk_customer_id" value="<?= htmlspecialchars((string) ($entity->sevdesk_customer_id ?? '')) ?>"><div class="form-text">Für diesen Mandanten ist keine SevDesk-Kontaktliste verfügbar.</div><?php endif; ?></div>
  <div class="col-12"><fieldset class="border rounded-3 p-3"><legend class="float-none w-auto px-2 fs-6 mb-0"><i class="fa-solid fa-file-invoice me-1" aria-hidden="true"></i>Rechnungsanschrift</legend><p class="small text-body-secondary mb-3">Optionaler Empfänger für SevDesk. Erst Straße sowie PLZ oder Ort überschreiben die beim SevDesk-Kontakt hinterlegte Anschrift.</p><?php if ((int) $entity->id > 0): ?><div id="sevdesk-address-options-<?= (int) $entity->id ?>" class="mb-3"><button class="btn btn-secondary btn-sm" type="button" hx-get="<?= htmlspecialchars(url_for('struktur/kunden/' . (int) $entity->id . '/sevdesk-adressen'), ENT_QUOTES) ?>" hx-target="#sevdesk-address-options-<?= (int) $entity->id ?>" hx-swap="innerHTML"><i class="fa-solid fa-address-book me-1" aria-hidden="true"></i>Rechnungsanschrift und Ansprechpartner aus SevDesk auswählen</button></div><?php else: ?><div class="form-text mb-3">Nach dem ersten Speichern können vorhandene SevDesk-Adressen und Ansprechpartner direkt ausgewählt werden.</div><?php endif; ?><input type="hidden" name="sevdesk_contact_address_id" value="<?= htmlspecialchars((string) ($entity->sevdesk_contact_address_id ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="sevdesk_contact_person_id" value="<?= htmlspecialchars((string) ($entity->sevdesk_contact_person_id ?? ''), ENT_QUOTES) ?>"><div class="row g-2"><div class="col-md-6"><label class="form-label">Empfänger / Organisation</label><input class="form-control" name="invoice_recipient_name" value="<?= htmlspecialchars((string) $entity->invoice_recipient_name) ?>" placeholder="z. B. Malteser Werke gGmbH"></div><div class="col-md-6"><label class="form-label">Ansprechpartner</label><input class="form-control" name="invoice_contact_name" value="<?= htmlspecialchars((string) $entity->invoice_contact_name) ?>" placeholder="z. B. Antoniuskolleg Neunkirchen"></div><div class="col-md-6"><label class="form-label">Straße / Hausnummer</label><input class="form-control" name="invoice_address_street" value="<?= htmlspecialchars((string) $entity->invoice_address_street) ?>"></div><div class="col-sm-4 col-md-2"><label class="form-label">PLZ</label><input class="form-control" name="invoice_address_zip" value="<?= htmlspecialchars((string) $entity->invoice_address_zip) ?>"></div><div class="col-sm-8 col-md-4"><label class="form-label">Ort</label><input class="form-control" name="invoice_address_city" value="<?= htmlspecialchars((string) $entity->invoice_address_city) ?>"></div><div class="col-md-6"><label class="form-label">Land</label><input class="form-control" name="invoice_address_country" value="<?= htmlspecialchars((string) ($entity->invoice_address_country ?: 'Deutschland')) ?>"></div></div></fieldset></div><?php endif; ?>
  <?php if ($type === 'floor'): ?>
  ...
  <div class="col-md-6"><details class="border rounded-3 h-100"><summary class="p-2 fw-semibold"><i class="fa-solid fa-sliders me-1" aria-hidden="true"></i>Zusatzattribute <span class="small text-body-secondary fw-normal">optional</span></summary><div class="p-2 pt-0"><?= render_template('metadata_editor_component.php', ['metadataName' => 'metadata_json', 'metadataId' => 'structure-metadata-' . $type . '-' . (int) $entity->id, 'metadataValue' => $metadataValue]) ?></div></details></div>
...
<?php if ($type === 'customer'): ?><script>if(!window.pruefappSevdeskAddressChoiceBound){window.pruefappSevdeskAddressChoiceBound=true;document.addEventListener('change',function(event){const select=event.target.closest('[data-sevdesk-address-choice]');const person=event.target.closest('[data-sevdesk-person-choice]');if(!select&&!person)return;const source=select||person;const option=source.options[source.selectedIndex];const form=source.closest('form');if(!option||!form)return;const set=(name,value)=>{const field=form.querySelector('[name="'+name+'"]');if(field)field.value=value||'';};if(select){set('sevdesk_contact_address_id',option.value);set('invoice_recipient_name',option.dataset.name);set('invoice_contact_name',option.dataset.name2);set('invoice_address_street',option.dataset.street);set('invoice_address_zip',option.dataset.zip);set('invoice_address_city',option.dataset.city);set('invoice_address_country',option.dataset.country);}else{set('sevdesk_contact_person_id',option.value);set('invoice_contact_name',option.dataset.name);}});}</script><?php endif; ?>
<?php };
