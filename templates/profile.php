...
/** @var array<int, array<string, string>> $certificates */
 ...
 $qualifications = $qualifications ?? []; $qualificationRequirements = $qualificationRequirements ?? []; $inspectionTypes = $inspectionTypes ?? []; $inspectionPermissions = $inspectionPermissions ?? []; $activeCompanionSessions = $activeCompanionSessions ?? []; $profileCompanionToken = $profileCompanionToken ?? ''; $canConfirmQualifications = (bool) ($canConfirmQualifications ?? false);
?>
...
<section class="card shadow-sm mb-4" id="companion-sessions" data-action-nav="Companion-Geräte" data-action-icon="fa-mobile-screen-button">
  ...
  <div class="card-body">
    ...
    <?php if ($canEdit): ?><form method="post" action="<?= htmlspecialchars($profileUrl, ENT_QUOTES) ?>" class="mb-3"><input type="hidden" name="action" value="create_companion_workspace"><button class="btn btn-primary" type="submit"><i class="fa-solid fa-qrcode me-1" aria-hidden="true"></i>Smartphone koppeln</button></form><?php endif; ?>
    <?php if ($profileCompanionToken !== ''): $profileCompanionUrl = url_for('companion/' . $profileCompanionToken); ?><div class="border rounded-3 p-3 mb-3"><div class="d-flex flex-wrap align-items-center gap-3"><div class="bg-white border rounded p-2"><canvas id="profile-companion-qr" width="176" height="176" aria-label="QR-Code zum Verbinden des Smartphones"></canvas></div><div><strong><i class="fa-solid fa-mobile-screen-button me-1" aria-hidden="true"></i>Smartphone verbinden</strong><p class="small text-body-secondary mb-2">Mit demselben Nutzerkonto öffnen. Die Verbindung bleibt bis zum Ende des Arbeitstags aktiv.</p><a class="btn btn-sm btn-secondary" href="<?= htmlspecialchars($profileCompanionUrl, ENT_QUOTES) ?>" target="_blank" rel="noopener"><i class="fa-solid fa-arrow-up-right-from-square me-1" aria-hidden="true"></i>Link öffnen</a></div></div></div><script>(() => { const canvas = document.getElementById('profile-companion-qr'); if (canvas && window.QRCode) QRCode.toCanvas(canvas, <?= json_encode($profileCompanionUrl) ?>, {width:176,margin:1,errorCorrectionLevel:'M'}, () => {}); })();</script><?php endif; ?>
