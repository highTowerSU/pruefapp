...
class StructureController
{
    ...
    public static function sevdeskCustomerAddresses(array $params, bool $isHx): array
    {
        ...
        try {
            ...
            $addresses = $client->contactAddresses((string) $customer->sevdesk_customer_id);
            $persons = $client->contactPersons((string) $customer->sevdesk_customer_id);
        } catch (Throwable $exception) {
            $error = $exception->getMessage();
        }
        return [200, ['Content-Type' => 'text/html; charset=utf-8'], render_template('customer_sevdesk_addresses.php', ['customer' => $customer, 'addresses' => $addresses, 'persons' => $persons, 'error' => $error])];
    }
    ...
    private static function save(string $type): array
    {
        ...
        if ($type === 'customer') {
            ...
            $entity->sevdesk_contact_person_id = trim((string) ($_POST['sevdesk_contact_person_id'] ?? ''));
        }
