...
final class ReportController
{
    ...
    private static function inspectionPdf(array $rows, string $title, array $branding): ?array
    {
        ...
        $resultCell = static function (string $result) use ($esc): string { $lower = mb_strtolower($result); $class = str_contains($lower, 'nicht') || str_contains($lower, 'durch') || str_contains($lower, 'fail') ? 'bad' : (str_contains($lower, 'bestanden') || in_array($lower, ['ok','ja','gut'], true) ? 'good' : 'pending'); return '<td class="ok"><span class="result ' . $class . '">' . $esc($result !== '' ? $result : 'OK') . '</span></td>'; };
