...
final class ReportController
{
    ...
    private static function inspectionPdf(array $rows, string $title, array $branding): ?array
    {
        ...
        $known = ['Prüfnummer','Datum','Prüfart','Prüfungstyp','Prüfer','Nächste Prüfung','Gerät','Ergebnis','Regiezeit','Regiebegründung','Inventarnummer','Geräteart','Hersteller','Typ','Wärmegerät','Raum-Nr.','__raw_json'];
        $items = [];
        ...
        $rawJson = json_decode((string) ($values['raw_json'] ?? ''), true) ?: [];
        $metadata = [['Prüfungs-Nr.', $values['Prüfnummer'] ?? ''], ['Datum', $values['Datum'] ?? ''], ['Art der Prüfung', $values['Prüfart'] ?? ($values['Prüfungstyp'] ?? '')], ['Prüfer', $values['Prüfer'] ?? ''], ['Nächste Prüfung', $values['Nächste Prüfung'] ?? ''], ['Gerät', $values['Gerät'] ?? ''], ['Inventar-Nr.', $values['Inventarnummer'] ?? ''], ['Geräteart', $values['Geräteart'] ?? ''], ['Hersteller', $values['Hersteller'] ?? ''], ['Typ', $values['Typ'] ?? ''], ['Wärmegerät', $values['Wärmegerät'] ?? ''], ['Raum-Nr.', $values['Raum-Nr.'] ?? '']];
        ...
        if (isset($rawJson['step0'])) { foreach ($rawJson as $key => $question) if (preg_match('/^step(\d+)$/', (string) $key, $m)) { $n=(int)$m[1]; $category=$n===0?'Inventarisierung':($n<=3?'Sichtprüfung':($n<=6?'Messung':($n===7?'Funktionsprüfung':'Organisatorische Hinweise'))); $result=(string)($rawJson['result'.$n]??''); $criterion=(string)($rawJson['criterion'.$n]??''); $html.='<tr><td>'.$esc($category).'</td><td>'.$esc((string)$question).'</td><td>'.$esc($criterion).'</td><td class="ok">'.$esc($result!==''?$result:'OK').'</td></tr>'; } } else foreach ($items as [$category,$question,$result,$criterion,$unused]) $html.='<tr><td>'.$esc($category).'</td><td>'.$esc($question).'</td><td>'.$esc($criterion).'</td><td class="ok">'.$esc($result).'</td></tr>';
