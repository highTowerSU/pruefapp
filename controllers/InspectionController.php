...
final class InspectionController
{
    ...
    public static function edit(array $params, bool $isHx): array
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Abrechnungsansicht gepflegt; alte Werte dürfen beim Speichern
            // einer Prüfung nicht versehentlich gelöscht werden.
            if (array_key_exists('billable', $_POST)) $inspection->billable = isset($_POST['billable']) ? 1 : 0;
    ...
    public static function detail(array $params, bool $isHx): array
    {
        ...
        foreach ($measurements as &$measurement) {
            ...
            $protectionClass = strtoupper(trim((string) ($inspection->protection_class ?? '')));
            $measurement['limit'] = in_array($name, ['RPE', 'RSL'], true) ? (in_array($protectionClass, ['II', 'III'], true) ? 'nicht anwendbar bei SK ' . $protectionClass : '≤ ' . rtrim(rtrim(number_format((float) ($inspection->rsl_limit_ohm ?: 0.3), 2, '.', ''), '0'), '.') . ' Ω' . ((float) ($inspection->cable_length_m ?? 0) > 0 ? ' (Kabellänge berücksichtigt)' : ' (Kabellänge nur bei >0,3 Ω erforderlich)')) : ($name === 'IPE' ? (in_array($protectionClass, ['II', 'III'], true) ? 'nicht anwendbar bei SK ' . $protectionClass : '≤ 3,5 mA') : ($name === 'RISO' ? '≥ ' . (((int) ($device->warming_device ?? 0) === 1) ? '0,3' : '1') . ' MΩ' . ((int) ($device->warming_device ?? 0) === 1 ? ' (Wärmegerät)' : '') : ''));
        }
        unset($measurement);
