...
function render_common_filter_panel(string $context, array $filters, array $data = []): string
{
    ...
    <form id="<?= $id ?>" class="common-filter-panel card card-body mb-4" method="get" action="<?= htmlspecialchars($action, ENT_QUOTES) ?>" hx-get="<?= htmlspecialchars($action, ENT_QUOTES) ?>" hx-target="<?= $context === 'billing' ? '#billing-content' : '#device-page' ?>" hx-select="<?= $context === 'billing' ? '#billing-content' : '#device-page' ?>" hx-swap="outerHTML" hx-push-url="true">
      ...
      <div id="<?= $id ?>-fields" class="row g-3 align-items-end">
        ...
        <div class="col-12 col-sm-6 col-md-4 col-xl-2"><label class="form-label">Kunde</label><select class="form-select" name="customer_id"><option value="">Alle Kunden</option><?= $options($customers, (string) ($filters['customer_id'] ?? '')) ?></select></div>
        <div class="col-12 col-sm-6 col-md-4 col-xl-2"><label class="form-label">Standort</label><select class="form-select" name="site_id"><option value="">Alle Standorte</option><?= $options($sites, (string) ($filters['site_id'] ?? '')) ?></select></div>
        <div class="col-12 col-sm-6 col-md-4 col-xl-2"><label class="form-label">Gebäude</label><select class="form-select" name="building_id"><option value="">Alle Gebäude</option><?= $options($buildings, (string) ($filters['building_id'] ?? '')) ?></select></div>
        <div class="col-12 col-sm-6 col-md-4 col-xl-2"><label class="form-label">Etage</label><select class="form-select" name="floor_id"><option value="">Alle Etagen</option><?= $options($floors, (string) ($filters['floor_id'] ?? '')) ?></select></div>
        <div class="col-12 col-sm-6 col-md-4 col-xl-2"><label class="form-label">Raum</label><select class="form-select" name="room_id"><option value="">Alle Räume</option><?= $options($rooms, (string) ($filters['room_id'] ?? ''), 'number') ?></select></div>
        <div class="col-6 col-md-3 col-xl-2"><label class="form-label">Von</label><input class="form-control" type="date" name="from" value="<?= $value('from') ?>"></div>
        <div class="col-6 col-md-3 col-xl-2"><label class="form-label">Bis</label><input class="form-control" type="date" name="to" value="<?= $value('to') ?>"></div>
        <?php if ($context === 'device'): ?>
        <div class="col-12 col-sm-6 col-md-3 col-xl-2"><label class="form-label">Prüfstatus</label><select class="form-select" name="inspection_status"><option value="">Alle Prüfungen</option><option value="failed"<?= $selected('inspection_status', 'failed') ?>>Nicht bestanden</option><option value="pending"<?= $selected('inspection_status', 'pending') ?>>Nicht abgeschlossen</option><option value="completed"<?= $selected('inspection_status', 'completed') ?>>Abgeschlossen</option></select></div>
