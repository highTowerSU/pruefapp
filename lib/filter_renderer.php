...
function render_common_filter_panel(string $context, array $filters, array $data = []): string
{
    ...
    <form id="<?= $id ?>" class="common-filter-panel card card-body mb-4" method="get" action="<?= htmlspecialchars($action, ENT_QUOTES) ?>" hx-get="<?= htmlspecialchars($action, ENT_QUOTES) ?>" hx-target="<?= $context === 'billing' ? '#billing-content' : '#device-page' ?>" hx-select="<?= $context === 'billing' ? '#billing-content' : '#device-page' ?>" hx-swap="outerHTML" hx-push-url="true">
      ...
      <div id="<?= $id ?>-fields" class="row g-3 align-items-end">
        ...
        <div class="col-12 col-md-auto d-flex flex-wrap gap-2"><button class="btn btn-primary" type="submit"><i class="fa-solid fa-filter me-1" aria-hidden="true"></i>Filtern</button><a class="btn btn-secondary" href="<?= htmlspecialchars($action, ENT_QUOTES) ?>" hx-get="<?= htmlspecialchars($action, ENT_QUOTES) ?>" hx-target="<?= $context === 'billing' ? '#billing-content' : '#device-page' ?>" hx-select="<?= $context === 'billing' ? '#billing-content' : '#device-page' ?>" hx-swap="outerHTML" hx-push-url="true"><i class="fa-solid fa-rotate-left me-1" aria-hidden="true"></i>Reset</a><?php if ($context === 'device'): ?><button type="button" class="btn btn-secondary" data-toggle-all-devices aria-expanded="false"><i class="fa-solid fa-angles-down me-1" aria-hidden="true"></i><span>Alle ausklappen</span></button><?php endif; ?></div>
      </div>
    </form>
    <?php
